...
class Depth_result(ABC):
    ...
    @abstractmethod
    def compute_h(self, x: float | np.ndarray, t: float | np.ndarray) -> None:
    ...
    @abstractmethod
    def compute_u(self, x: float | np.ndarray, t: float | np.ndarray) -> None:
        ...
        for t in T:
            ...
            for i in x:
                if i <= self.xa(t):
                    ...
                    sub_h.append(
                        (4 / (9 * self._g))
                        * (np.sqrt(self._g * self._h0) - ((i - self._x0) / (2 * t)))
                        ** 2
                    )
                else:
                    sub_h.append(0)
        ...
        for t in T:
            ...
            for i in x:
                if i <= self.xa(t):
                    ...
                    sub_u.append(
                        (2 / 3) * (((i - self._x0) / t) + np.sqrt(self._g * self._h0))
                    )
                else:
                    sub_u.append(np.nan)
            u.append(sub_u)
...
class Stoker_SWASHES_wet(Depth_result):
    ...
    def __init__(self, x_0: float, h_0: float, h_r: float, h_m: float = None):
        super().__init__()
        self._x0 = x_0
        self._h0 = h_0
        self._hr = h_r
    ...
    def xc(self, t: float) -> float:
        ...
        return self._x0 + (
            t
            * (
                ((2 * self._cm**2) * (np.sqrt(self._g * self._h0) - self._cm))
                / ((self._cm**2) - (self._g * self._hr))
            )
        )
    ...
    def equation_cm(self, cm) -> float:
        ...
        """
        return -8 * self._g * self._hr * cm**2 * (self._g * self._h0 - cm**2) ** 2 + (
            cm**2 - self._g * self._hr
        ) ** 2 * (cm**2 + self._g * self._hr)
    ...
    def compute_cm(self) -> None:
        ...
        for guess in guesses:
            ...
            if abs(self.equation_cm(sol)) < 1e-6 and not any(
                np.isclose(sol, s, atol=1e-6) for s in solutions
            ):
                ...
                for i in x:
                    ...
                    # elif i > self.xa(t) and i <= self.xb(t):
                        ...
                        h.append(
                            ...
                            * (np.sqrt(self._g * self._h0) - ((i - self._x0) / (2 * T)))
                            ** 2
                        )  # i-x0 and not i to recenter the breach of the dam at x=0.
            ...
            if isinstance(T, float):
                ...
                for i in x:
                    if i <= self.xa(T):
                        ...
                        u.append(
                            (2 / 3)
                            * (((i - self._x0) / T) + np.sqrt(self._g * self._h0))
                        )
...
class Stoker_SARKHOSH_wet(Depth_result):
    ...
    def __init__(
        ...
        if self._hr == 0:
            ...
            self._hm = (
                0.5
                * self._hr
                * (np.sqrt(1 + 8 * self._cm**2 / np.sqrt(self._g * self._hr) ** 2) - 1)
            )
    ...
    def compute_cm(self) -> float:
        ...
        while abs(f_cm / cm) > 1e-10:
            ...
            f_cm = cm * self._hr - self._hr * (root_term - 1) * (
                cm / 2 - np.sqrt(self._g * self._h0) + inner_sqrt
            )
            df_cm = (
                self._hr
                - self._hr
                * (
                    (2 * cm * self._g * self._hr)
                    / (np.sqrt(self._g * self._hr) ** 2 * root_term * inner_sqrt)
                    + 0.5
                )
                * (root_term - 1)
                - (
                ...
                / (np.sqrt(self._g * self._hr) ** 2 * root_term)
            )
    ...
    def compute_h(self, x: float | np.ndarray, T: float | np.ndarray) -> None:
        ...
        for t in T:
            ...
            for i in x:
                if t == 0:
                    h_val = (2 * np.sqrt(self._g * self._h0) - 1e18) ** 2 / (
                        9 * self._g
                    )
                else:
                    h_val = (2 * np.sqrt(self._g * self._h0) - (i / t)) ** 2 / (
                        9 * self._g
                    )
                ...
                if self._hm == 0 and h_val > sub_h[-1]:
                    ...
                    if (
                        self.xb(self._hm, t) < i <= self.xc(self._cm, t)
                    ) and h_val <= self._hm:
                        h_val = self._hm
...
class Mangeney_dry(Depth_result):
    ...
    def compute_m(self) -> float:
        ...
        """
        return (self._g * np.sin(self._theta)) - (
            self._g * np.cos(self._theta) * np.tan(self._delta)
        )
        ...
        for t in T:
            ...
            for i in x:
                if i <= self.xa(t):
                    ...
                    sub_h.append(
                        (1 / (9 * self._g * np.cos(self._theta)))
                        * ((-(i - self._x0) / t) + (2 * self._c0) + (0.5 * t * self._m))
                        ** 2
                    )
...
class Dressler_dry(Depth_result):
    ...
    def __init__(self, x_0: float, h_0: float, C: float = 40):
        super().__init__()
        self._x0 = x_0
        self._h0 = h_0
        self._c = C
            ...
            for i in x:
                if i <= self.xa(t):
                    ...
                    if t == 0:
                        t = 1e-18
                    term = (
                        ((2 / 3) * np.sqrt(self._g * self._h0))
                        - ((i - self._x0) / (3 * t))
                        + ((self._g**2) / (self._c**2)) * self.alpha1(i, t) * t
                    )
                    h_val = (1 / self._g) * term**2

                    if sub_u[-1] is np.nan:
                        sub_u[-1] = 0
                    u_val = (2 / 3) * np.sqrt(self._g * self._h0) * (
                        1 + (i - self._x0) / (np.sqrt(self._g * self._h0) * t)
                    ) + ((self._g**2) / (self._c**2)) * self.alpha2(i, t) * t
                    ...
                    if xt is not None:
                        u_val = ut
                        h_val = (-b - np.sqrt((b**2) - 4.0 * a * (c - i))) / (2 * a)
...
class Chanson_dry(Depth_result):
    ...
    def __init__(self, h_0: float, x_0: float, f: float):
        super().__init__()
        self._h0 = h_0
        self._x0 = x_0
        self._f = f
    ...
    def xb(self, t: float) -> float:
        ...
        return self._x0 + ((3 * cf) / (2 * np.sqrt(self._g * self._h0)) - 1) * (
            t * np.sqrt(self._g * self._h0)
        )
    ...
    def xc(self, t: float) -> float:
        ...
        term1 = (
            ((1.5 * (cf / np.sqrt(self._g * self._h0))) - 1)
            * np.sqrt(self._g / self._h0)
            * t
        )
        term2 = (4 / (self._f * ((cf**2) / (self._g * self._h0)))) * (
            1 - 0.5 * (cf / np.sqrt(self._g * self._h0))
        ) ** 4
    ...
    def compute_cf(self, t: float) -> float:
        ...
        coeffs = [
            ...
            (
                ...
                * (
                    0.75
                    - ((3 * self._f * t * np.sqrt(self._g)) / (8 * np.sqrt(self._h0)))
                )
            ),
            12,
            -8,
        ...
        for t in T:
            ...
            for i in x:
                if i <= self.xa(t):
                    ...
                    term = (
                        (self._f / 4)
                        * ((cf**2) / (self._g * self._h0))
                        * ((self.xc(t) - (i)) / self._h0)
                    )
                    val = np.sqrt(term) * self._h0
                    sub_h.append(val)
...
class Coussot_shape(Shape_result):
    ...
    def x_to_X(self, x: float) -> float:
        ...
        if self._theta == 0:
            ...
            return (
                self._rho * self._g * x * np.sin(self._theta) * np.sin(self._theta)
            ) / (self._tau * np.cos(self._theta))
    ...
    def X_to_x(self, X: float) -> float:
        ...
        if self._theta == 0:
            ...
            return (X * self._tau * np.cos(self._theta)) / (
                self._rho * self._g * np.sin(self._theta) * np.sin(self._theta)
            )
...
class Front_result:
    ...
    def xf_mangeney(self, t: float, delta: float, theta: float = 0) -> float:
        r"""
        ...
        m = self._g * np.sin(theta_rad) - (
            self._g * np.cos(theta_rad) * np.tan(delta_rad)
        )
    ...
    def xf_ritter(self, t: float) -> float:
        r"""
    ...
    def xf_stoker(self, t: float, hr: float) -> float:
        r"""
        ...
        while abs(f_cm / cm) > 1e-10:
            ...
            df_cm = (
                ...
                * (
                    (2 * cm * self._g * hr)
                    / (np.sqrt(self._g * hr) ** 2 * root_term * inner_sqrt)
                    + 0.5
                )
                * (root_term - 1)
                - (8 * cm * hr * (cm / 2 - np.sqrt(self._g * self._h0) + inner_sqrt))
                / (np.sqrt(self._g * hr) ** 2 * root_term)
            )
    ...
    def xf_chanson(self, t: float, f: float) -> float:
        r"""
        ...
        coeffs = [
            1,
            (-8 * (0.75 - ((3 * f * t * np.sqrt(self._g)) / (8 * np.sqrt(self._h0))))),
            12,
            -8,
        ]
        roots = np.roots(coeffs)
        ...
        term2 = (4 / (f * ((cf**2) / (self._g * self._h0)))) * (
            1 - 0.5 * (cf / np.sqrt(self._g * self._h0))
        ) ** 4
