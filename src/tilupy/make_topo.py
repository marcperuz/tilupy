...
def channel(nx=None, ny=None, dx=None, dy=None,
    ...
    # Height above flattened topography is a channel in alpha(x)*(y-thalweg(x))**2,

    # alpha(x) is 1/2R in the channel, and depends on a transition function in the transition
    # zones
...
#        for i in np.arange(0,nbends-1):
#            x1=xstart_channel+step/2+i*step
#            x2=xstart_channel+step/2+(i+1)*step
#            ind=(xtopo>=x1) & (xtopo<x2)
#            thalweg[ind]=(-1)**(i)*mid_bend(xtopo[ind],x1,x2)
        if nbends > 1:
    ...
    slope_angle[ind] = (xtopo[ind]-xstart_trans)/(xstart_channel -
                                                  xstart_trans)*(theta_channel-theta_start)+theta_start

    ind = (xtopo >= xend_channel) & (xtopo < xend_trans)
    slope_angle[ind] = (xtopo[ind]-xend_trans)/(xend_channel -
                                                xend_trans)*(theta_channel-theta_end)+theta_end
    ...
    # Vector normal to topography in cartesian coordinates (nx,ny,nz)=(-sin(theta),0,cos(theta))
...
if __name__ == '__main__':
    ...
    theta_channel = 10
    theta_end = 10
    channel(nx, ny, xmin=xmin, xmax=xmax, ymax=ymax,
