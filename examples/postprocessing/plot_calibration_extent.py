...
import os
import numpy as np
import pytopomap.plot #For representation only
import tilupy.read
...
x, y, obs_extent = tilupy.raster.read_ascii(os.path.join(folder_simus, 'calibration', 'extent_example.asc'))
...
front.plot(ax=axe, marker='d', markersize=50,
           edgecolor='white', facecolor='b', zorder=5)
...
deltas = [10, 15, 20, 25] 
parameter_files = ['delta_{:05.2f}'.format(delta).replace('.','p')+'.txt' 
                   for delta in deltas]

# %%
# Construct list of simulations used for calibration
simus = [tilupy.read.get_results('shaltop', folder=folder_simus, file_params=file)
