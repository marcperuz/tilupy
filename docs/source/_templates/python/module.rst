...
.. autoapisummary::
   ...
   {{ exception.id }}
            ...
            {# --- Functions moved before Classes --- #}
...
.. autoapisummary::
   ...
   {{ function.id }}
            ...
            {# --- Classes now appear after Functions --- #}
...
{{ "-" * obj.type|length }}---------
               ...
               {% set inline_attributes = this_page_children|selectattr("type", "equalto", "data")|list %}
               {% if inline_attributes %}
Attributes
^^^^^^^^^^

                  {% for attr in inline_attributes %}
{{ attr.render()|indent(0) }}
                  {% endfor %}
               {% endif %}

               {# Sous-section : Functions #}
               {% set inline_functions = this_page_children|selectattr("type", "equalto", "function")|list %}
               {% if inline_functions %}
Functions
^^^^^^^^^

                  {% for func in inline_functions %}
{{ func.render()|indent(0) }}
                  {% endfor %}
               {% endif %}

               {# Sous-section : Classes #}
               {% set inline_classes = this_page_children|selectattr("type", "equalto", "class")|list %}
               {% if inline_classes %}
Classes
^^^^^^^
